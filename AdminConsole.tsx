...
export function AdminConsole({ onBack }: { onBack: () => void }) {
  ...
  async function reindexWebsite() {
    if (!knowledge.url) {
      setNotice("Enter an approved website URL before starting a refresh.");
      return;
    }
    try {
      const result = await api<{ visited: number; indexed: number }>("/api/admin/knowledge/reindex", { method: "POST", body: JSON.stringify({ url: knowledge.url }) });
      setNotice(`Knowledge refresh complete: visited ${result.visited} page(s) and indexed ${result.indexed} changed source(s).`);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Unable to refresh approved website knowledge.");
    }
  }
